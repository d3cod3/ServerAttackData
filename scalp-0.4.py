...
def scalper(access, filters, preferences = [], output = "text"):
	...
	if n > 0:
		...
		elif 'stdout' in preferences['output']:
			generate_STDoutput(flag, short_name, filters, preferences['odir'])
...
def generate_STDoutput(flag, access, filters, odir):
	for attack_type in flag:
		if attack_type in names:
			print ("Attack %s (%s)\n" % (names[attack_type], attack_type))
		else:
			print ("Attack type: %s\n" % attack_type)
		impacts = list(flag[attack_type])
...
def main(argc, argv):
	...
	preferences = {
		'attack_type' : [],
		'period' : {
			'start' : [0o1, 0o0, 0o000, 0o0, 0o0, 0o0],# day, month, year, hour, minute, second
